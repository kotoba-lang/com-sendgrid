(ns sendgrid.main
  "Kotodama WASM entrypoint for the Sendgrid clean-room actor (L4) — Clojure port.
  ...
  facts (`sendgrid.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sendgrid")
(def tier "L4")
...
(def entity-specs
  [{:entity "Contact"          :plural "contacts"          :id-prefix "sendgrid_con"
    :fields [:email :firstName :lastName :country :externalId] :required [:email :firstName]
    :coerce {}                                   :refs {}}
   {:entity "Template"         :plural "templates"         :id-prefix "sendgrid_tem"
    :fields [:name :generation]                  :required [:name :generation]
    :coerce {}                                   :refs {}}
   {:entity "SingleSend"       :plural "singlesends"       :id-prefix "sendgrid_sin"
    :fields [:name :status :templateId :senderId] :required [:name :status]
    :coerce {}                                   :refs {:templateId "Template" :senderId "Sender"}}
   {:entity "Bounce"           :plural "bounces"           :id-prefix "sendgrid_bou"
    :fields [:email :reason :status]             :required [:email :reason]
    :coerce {}                                   :refs {}}
   {:entity "UnsubscribeGroup" :plural "unsubscribegroups" :id-prefix "sendgrid_uns"
    :fields [:name :description :isDefault]      :required [:name :description]
    :coerce {:isDefault :bool}                   :refs {}}
   {:entity "Sender"           :plural "senders"           :id-prefix "sendgrid_sen"
    :fields [:nickname :fromEmail :fromName :verified] :required [:nickname :fromEmail]
...
(defn emit-facts
  "EAVT facts for one record: {\"sendgrid.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sendgrid-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sendgrid-compat/ready)
